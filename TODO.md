...
## Markdown / weaver snippets

The ids inside `<!-- weaver ... -->` snippet queries are indexed as references
into the defining YAML (see [server/src/markdown.ts](server/src/markdown.ts)).

- [ ] Navigate from attribute names rendered as ``[`key`](...)`` links in the
      autogenerated snippet body / attribute tables to their YAML definition
      (read-only; never renamed, since weaver regenerates that text).
- [ ] Diagnose a snippet query that selects from an unknown collection or uses an
      unrecognized selector field.
...
- [ ] **Rename**
  ...
  - [x] Rewrite ids in `<!-- weaver ... -->` snippet queries in Markdown docs
        across the workspace when the definition is renamed (see _Markdown / weaver
        snippets_ below).
