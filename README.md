...
- **Ask about your conventions in chat** — each registry in the workspace is
  exposed to Copilot agent mode as a [Weaver](https://github.com/open-telemetry/weaver)
...
| `semconv.weaver.path`   | `""`    | Path to the `weaver` executable for the MCP server and registry check. When empty, `weaver` is looked up on `PATH`, then run via the pinned `otel/weaver` Docker image.                                            |
