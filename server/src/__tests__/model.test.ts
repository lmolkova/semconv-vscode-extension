...
/** Slice the original text by an LSP range to recover the token text. */
function tokenText(text: string, range: { start: any; end: any }): string {
  const lines = text.split('\n');
  if (range.start.line === range.end.line) {
    return lines[range.start.line].slice(range.start.character, range.end.character);
  }
  return text; // multi-line not needed in these assertions
}
...
describe('extract – references', () => {
  it('captures ref, ref_group, entity_associations and refinement refs', () => {
    ...
    const providerRef = refs.find((r) => r.refKind === 'attribute_ref' && r.id === 'gen_ai.provider.name');
